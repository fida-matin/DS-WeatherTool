...
SRC_DIR = src
BIN_DIR = bin
TEST_DIR = tests  # Directory for compiled test classes

# Find all .java files in src except test files
SRC = $(shell find $(SRC_DIR) -name '*.java' ! -name '*Test.java')

# Find all test .java files
TEST_SRC = $(shell find $(SRC_DIR) -name '*Test.java')

# Convert .java file paths to .class file paths in BIN_DIR
CLASSES = $(SRC:$(SRC_DIR)/%.java=$(BIN_DIR)/%.class)

# Convert test .java file paths to .class file paths in TEST_DIR
TEST_CLASSES = $(TEST_SRC:$(SRC_DIR)/%.java=$(TEST_DIR)/%.class)
...
# Rule for compiling each source .java file to .class file
$(BIN_DIR)/%.class: $(SRC_DIR)/%.java
	$(JC) $(JFLAGS) -d $(BIN_DIR) $<

# Rule for compiling each test .java file to .class file
$(TEST_DIR)/%.class: $(SRC_DIR)/%.java
	$(JC) $(JFLAGS) -d $(TEST_DIR) $<

# Run tests using JUnit
test: tests
	java -cp $(BIN_DIR):$(TEST_DIR):lib/junit-platform-console-standalone-1.7.2.jar org.junit.platform.console.ConsoleLauncher --scan-class-path
